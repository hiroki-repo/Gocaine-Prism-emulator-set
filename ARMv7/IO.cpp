...
uint16_t IO::ld_halfword(int addr)
{
    int rval = check_portl(addr);
    if (rval != -555)
        return rval;
...
uint8_t IO::st_byte(int addr, uint8_t onebyte)
{
...
uint16_t IO::st_halfword(int addr, uint16_t halfword)
{
...
uint32_t IO::st_word(int addr, uint32_t word)
{
...
uint32_t IO::ld_data(int addr)
{
    int naddr = addr - 0xFFF7580;
    return port_data[naddr];
}
uint32_t IO::st_data(uint32_t addr, uint32_t data)
{
    int naddr        = addr - 0xFFF7580;
